...
for dataset in fb_yago_subsets.keys():
    print('Learning Ridle Representations on', dataset)
    # Loading Files
    df = pd.read_pickle('./dataset/{}/dataset.pkl'.format(dataset))[['S', 'P']].drop_duplicates()
    ...
    r.to_csv('./dataset/{}/embedding.csv'.format(dataset), index=False)
